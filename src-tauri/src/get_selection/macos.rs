...
pub fn get_text() -> String {
    match get_selected_text_by_ax() {
        Ok(text) => {
            if !text.is_empty() {
                ...
                println!("get_selected_text_by_ax is empty");
            }
        }
        Err(err) => {
    ...
    println!("fallback to get_text_by_clipboard");
    match get_text_by_clipboard() {
        Ok(text) => {
            if !text.is_empty() {
                return text;
            } else {
                println!("get_text_by_clipboard is empty");
            }
        }
        Err(err) => {
            println!("get_text_by_clipboard error:{}", err);
        }
    }
    // Return Empty String
    String::new()
